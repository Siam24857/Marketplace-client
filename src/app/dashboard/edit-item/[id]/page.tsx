...
import { useItem, useUpdateItem } from '@/hooks/useQueries';
...
export default function EditItemPage() {
  ...
  useEffect(() => {
    if (itemData?.data && !initialized) {
      const item = itemData.data as any;
      reset({
        title: item.title,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await updateItem.mutateAsync({ id, data: { ...data, price: parseFloat(data.price) } });
  ...
  if (authLoading || !user || itemLoading) return null;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Edit Item</h1>
        <p className="text-gray-500 dark:text-gray-400 mb-8">Update your product listing.</p>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex gap-4">
              <Button type="submit" isLoading={updateItem.isPending} className="flex-1">Save Changes</Button>
